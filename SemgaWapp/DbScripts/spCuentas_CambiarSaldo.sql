...
BEGIN
    ...
    BEGIN TRY
        ...
        DECLARE @snImputable BIT;

        SELECT @SaldoAnterior = [Saldo], @Cuenta = [Cuenta], @snImputable = [snImputable]
        FROM dbo.tbCuentas
        WHERE [ID] = @ID
          AND (snEliminado = 0 OR snEliminado IS NULL);
        ...
        -- Solo las cuentas imputables pueden modificar su saldo
        IF ISNULL(@snImputable, 0) = 0
        BEGIN
            SELECT 'ERROR' AS Resultado, 'La cuenta no es imputable; su saldo no puede modificarse.' AS Mensaje;
            RETURN;
        END
