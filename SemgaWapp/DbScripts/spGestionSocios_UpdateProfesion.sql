-- =============================================
-- ACTUALIZAR STORED PROCEDURES PARA PROFESION
-- =============================================
...
ALTER PROCEDURE [dbo].[spGestionSocios_ObtenerSocios]
...
BEGIN
    ...
    BEGIN TRY
        SELECT
            ...
            a.Ocupacion, a.NivelEstudio, tn.Descripcion AS NivelEstudioDescripcion,
            a.Profesion, tp.Descripcion AS ProfesionDescripcion, a.FechaCreacion, a.UsuarioCrea,
            a.FechaModificacion, a.UsuarioModifica, a.snEliminado
        FROM tbAsociados a
        LEFT JOIN tbTipoAsociado ta ON a.IdTipoAsociado = ta.IdTipoAsociado
        LEFT JOIN tbNivelesEstudio tn ON a.NivelEstudio = tn.Code
        LEFT JOIN tbProfesiones tp ON a.Profesion = tp.Code
...
ALTER PROCEDURE [dbo].[spGestionSocios_ObtenerSocioPorNumero]
    @NumeroAsociado INT
AS
BEGIN
    SET NOCOUNT ON;
...
ALTER PROCEDURE [dbo].[spGestionSocios_CrearSocio]
    ...
    @Ocupacion NVARCHAR(100) = NULL, @NivelEstudio INT = NULL, @Profesion INT = NULL,
...
ALTER PROCEDURE [dbo].[spGestionSocios_ActualizarSocio]
...
BEGIN
    ...
    BEGIN TRY
        UPDATE tbAsociados SET
            IdTipoAsociado = ISNULL(@IdTipoAsociado, IdTipoAsociado),
            ...
            TelefonoResidencia = ISNULL(@TelefonoResidencia, TelefonoResidencia),
            TelefonoCelular = ISNULL(@TelefonoCelular, TelefonoCelular),
            TelefonoFamiliar = ISNULL(@TelefonoFamiliar, TelefonoFamiliar),
            CorreoElectronico = ISNULL(@CorreoElectronico, CorreoElectronico),
            Sexo = ISNULL(@Sexo, Sexo),
            FechaNacimiento = ISNULL(@FechaNacimiento, FechaNacimiento),
            ProvinciaResidencia = ISNULL(@ProvinciaResidencia, ProvinciaResidencia),
            DistritoResidencia = ISNULL(@DistritoResidencia, DistritoResidencia),
            CorregimientoResidencia = ISNULL(@CorregimientoResidencia, CorregimientoResidencia),
            DireccionResidencia = ISNULL(@DireccionResidencia, DireccionResidencia),
            ProvinciaTrabajo = ISNULL(@ProvinciaTrabajo, ProvinciaTrabajo),
            DistritoTrabajo = ISNULL(@DistritoTrabajo, DistritoTrabajo),
            CorregimientoTrabajo = ISNULL(@CorregimientoTrabajo, CorregimientoTrabajo),
            DireccionTrabajo = ISNULL(@DireccionTrabajo, DireccionTrabajo),
            LugarTrabajo = ISNULL(@LugarTrabajo, LugarTrabajo),
            Ocupacion = ISNULL(@Ocupacion, Ocupacion),
            NivelEstudio = ISNULL(@NivelEstudio, NivelEstudio),
...
PRINT 'Stored procedures actualizados exitosamente para manejar Profesion como integer';
