...
ALTER   PROCEDURE [dbo].[spAuxiliares_ObtenerAuxiliares]
    @IDAuxiliar INT = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        WHERE IsNull(a.snEliminado, 0) = 0
          ...
          -- ============================
          -- FILTRO POR @FiltroAsoc
          -- ============================
          AND (
                @FiltroAsoc IS NULL
                OR (
                      TRY_CONVERT(int, @FiltroAsoc) IS NOT NULL
                      AND a.NumeroAsociado = TRY_CONVERT(int, @FiltroAsoc)
                   )
                OR (
                      TRY_CONVERT(int, @FiltroAsoc) IS NULL
                      AND (
                             s.Nombre LIKE '%' + @FiltroAsoc + '%'
                          OR s.Apellido LIKE '%' + @FiltroAsoc + '%'
                          OR s.NumeroIdentificacion LIKE '%' + @FiltroAsoc + '%'
                          )
                   )
              )
